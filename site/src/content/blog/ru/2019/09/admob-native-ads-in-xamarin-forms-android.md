---
title: "AdMob Native Ads в Xamarin Forms (Android)"
description: "Пошаговое руководство по реализации AdMob Native Ads в приложении Xamarin Forms для Android с использованием custom renderer."
pubDate: 2019-09-20
...
---
До недавнего времени NativeAds находились в закрытой бете и доступ к этому формату был только у некоторых разработчиков, но теперь он доступен всем. Вот как добавить такой блок в ваше Android-приложение Xamarin Forms с помощью custom renderer.

## Шаг 1. Создайте view, выступающее placeholder для рекламы

У него две роли. С одной стороны, это класс view, к которому можно привязать custom renderers, а с другой - placeholder, резервирующий место и показывающий текст "AD", чтобы UI не дёргалась при загрузке рекламы.
...
Замечание: я зашил высоту 360 в view ради удобства, потому что использую её в ListView, где у всех элементов одинаковая высота. Это можно убрать и задать высоту в XAML или при добавлении элемента.

## Шаг 2. Создайте layout рекламы

Для этого перейдите в вашем Android-проекте в Resources > layout и создайте новый файл с именем ad\_unified.axml. Вы можете использовать следующее как стартовый layout и менять под себя.
...
Меняйте как угодно, но оставьте все элементы текста/изображения/медиа. Сейчас SDK выводит предупреждение, если настроены не все элементы, а в будущем это будет запрещено.

## Шаг 3. Добавьте custom renderer

Этот custom renderer инициирует запрос рекламы и развернёт указанный layout в реальный ad-control.
...
Не забудьте поменять namespaces и обновить ID рекламы. Для тестов можно использовать эти примеры ad ID, предоставленные Google:

-   Native Advanced: `ca-app-pub-3940256099942544/2247696110`
-   Native Advanced Video: `ca-app-pub-3940256099942544/1044960115`

## Шаг 4. Покажите рекламу на странице

В любом месте вашего XAML просто добавьте:

```xml
<controls:NativeAdView HeightRequest="360" />
```

Либо, если хотите использовать это, например, в ListView, создайте новый класс AdPlaceholder и вставляйте его в список каждые, скажем, 5 элементов. Затем с помощью data template selector выбирайте либо реальный элемент списка, либо ad-view, например так:
...
Только не забудьте подвязать data template selector к вашему ListView.

Вот и всё. До iOS-renderer пока руки не дошли, и потребуется ещё несколько недель, чтобы взяться. Если кто-то сделает - пишите в комментариях.

Надеюсь, это поможет. Если есть вопросы - оставляйте их в комментариях!
