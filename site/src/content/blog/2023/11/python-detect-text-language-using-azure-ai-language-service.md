...
credential = AzureKeyCredential('98c1961504db412c9fd36d15984c9d9e')
ai_client = TextAnalyticsClient(endpoint='https://my-service.cognitiveservices.azure.com/', credential=credential)
```
...
The `TextAnalyticsClient` comes with a `detect_language` method with overides accepting either a `List[str]`, a `List[DetectLanguageInput]`, or a `List[Dict[str, str]]`.
