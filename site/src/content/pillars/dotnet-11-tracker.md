...
Start at the top: each post covers one change, and older ones stay useful. RC 1 is newest: [reproducible container images](/2026/09/dotnet-11-rc-1-reproducible-container-images-source-date-epoch/), so a commit always yields the same digest, [Process can SIGTERM a child without P/Invoke](/2026/09/dotnet-11-rc-1-process-signal-exit-status/), and [SignalR swaps an expiring token without dropping the connection](/2026/09/signalr-authentication-refresh-finalized-dotnet-11-rc-1/). In the runtime, [the JIT devirtualizes generic virtual methods](/2026/09/dotnet-11-jit-devirtualizes-generic-virtual-methods/) and takes the allocation with it. From Preview 7, [MSBuild server on by default](/2026/08/msbuild-server-on-by-default-dotnet-11-preview-7/) and [encrypted ZIPs](/2026/08/dotnet-11-preview-7-password-protected-zip-archives/) still matter. The breaking change to know first is [the minimum CPU baseline rising to x86-64-v2](/2026/06/dotnet-11-minimum-cpu-baseline-x86-64-v2/), which won't start on older hardware.
