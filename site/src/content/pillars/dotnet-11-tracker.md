...
This pillar collects everything I've written about **.NET 11**: previews, runtime changes, GC and JIT work, and the new BCL surface. I keep it current as each preview drops.

## What to read first

New here? Start at the top: each post covers one change, and older ones stay useful. Preview 7 is newest: [MSBuild server is on by default](/2026/08/msbuild-server-on-by-default-dotnet-11-preview-7/), so back-to-back builds reuse a warm worker, [System.IO.Compression reads and writes encrypted ZIPs](/2026/08/dotnet-11-preview-7-password-protected-zip-archives/), and [Blazor Server circuits pause when the tab goes idle](/2026/08/blazor-auto-pause-idle-circuits-dotnet-11-preview-7/). From Preview 6, [automatic CSRF protection on by default](/2026/07/aspnetcore-11-automatic-csrf-protection-fetch-metadata-preview-6/) and [union types in System.Text.Json](/2026/07/serialize-csharp-union-types-with-system-text-json-dotnet-11-preview-6/) still matter. Outside the previews, [Aspire 13.5 puts a terminal in the dashboard](/2026/08/aspire-13-5-withterminal-interactive-shells-in-the-dashboard/). The breaking change to know first is [the minimum CPU baseline rising to x86-64-v2](/2026/06/dotnet-11-minimum-cpu-baseline-x86-64-v2/), which won't start on older hardware.
