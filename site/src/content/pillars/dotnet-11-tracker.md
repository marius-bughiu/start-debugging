...
If you're new to the .NET 11 story, start with the newest posts near the top of the list; each covers a single change, and older posts stay useful because the feature set is cumulative. Preview 6 is the newest drop: [ASP.NET Core turns on automatic CSRF protection by default](/2026/07/aspnetcore-11-automatic-csrf-protection-fetch-metadata-preview-6/), which rejects unsafe cross-origin browser requests via the Sec-Fetch-Site header, and [C# 15 extension indexers round out extension members](/2026/07/csharp-15-extension-indexers-dotnet-11-preview-6/) are the headline changes. From Preview 5, the BCL wins that still matter are [System.Text.Json writing JSON Lines](/2026/06/system-text-json-json-lines-serialization-dotnet-11-preview-5/), [LINQ's FullJoin and selector-free joins](/2026/06/linq-fulljoin-tuple-returning-joins-dotnet-11-preview-5/), and [in-box X25519 key agreement](/2026/06/dotnet-11-x25519-key-agreement-preview-5/). The breaking change to know before you upgrade is [the minimum CPU baseline rising to x86-64-v2](/2026/06/dotnet-11-minimum-cpu-baseline-x86-64-v2/), which can refuse to start on older hardware.
