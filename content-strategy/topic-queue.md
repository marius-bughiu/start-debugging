...
- How to call a stored procedure and map its results in EF Core 11 → slug: 2026/08/how-to-call-a-stored-procedure-and-map-its-results-in-ef-core-11
...
- Fix: `InvalidOperationException: JavaScript interop calls cannot be issued at this time` during Blazor prerendering → slug: 2026/07/fix-javascript-interop-calls-cannot-be-issued-at-this-time-blazor-prerendering
...
- Fix: `Could not load file or assembly 'Microsoft.Data.SqlClient, Version=7.0.0.0'` after updating `Microsoft.EntityFrameworkCore.SqlServer` → slug: 2026/09/fix-could-not-load-file-or-assembly-microsoft-data-sqlclient-version-7-0-0-0 (premise sharpened: EF Core 10.0.x DLLs reference SqlClient 6.0.0.0, only EF Core 11 references 7.0.0.0; causes are a leftover 6.x pin with NU1605/NU1109 suppressed, a deploy dropping runtimes/, or a plug-in host; dotnet/efcore#38845)
- Fix: `InvalidOperationException: Nullable object must have a value` in an EF Core 11 projection → slug: 2026/09/fix-nullable-object-must-have-a-value-in-an-ef-core-11-projection (measured on EF Core 11 RC 1 vs 10.0.12: optional-nav/Max/Average/DefaultIfEmpty-DTO causes unchanged; LeftJoin+GroupBy fixed in 11 by #38479; JSON complex collection + collection nav is an 11 regression, #38928, fix #38932, backport #38948 pending)
- Fix: `flutter doctor --android-licenses` fails with Android SDK Command-line Tools 23 → slug: 2026/09/fix-flutter-doctor-android-licenses-fails-with-cmdline-tools-23 (measured: cmdline-tools 23.0 `sdkmanager --licenses` prints only a deprecation warning; Flutter <=3.47.2 reports 'license status unknown'; fixed in 3.47.3 / 3.48.0-0.4.pre by #191554, no 3.44 backport)
- Fix: ASP.NET Core API endpoints return 401 instead of redirecting to the login page after upgrading to .NET 10 → slug: 2026/09/fix-aspnetcore-api-endpoints-return-401-instead-of-redirecting-to-login-dotnet-10 (measured 9.0.20 vs 10.0.10: IDisableCookieRedirectMetadata, the MS Learn page's IApiEndpointMetadata was renamed before GA in #63283; undocumented IgnoreRedirectMetadata AppContext switch verified)
...
- Fix: `Daemon compilation failed: null` in a Flutter Android Gradle build → slug: 2026/09/fix-daemon-compilation-failed-null-in-a-flutter-android-gradle-build (root cause pinned: Kotlin IC RelocatableFileToPathConverter uses File.relativeTo(rootProjectDir), which throws "different roots" when pub cache and project are on different Windows drives, KT-63983 since Kotlin 1.9.20; KGP <=2.3.0 falls back to non-incremental in-process compile so the APK builds, KGP 2.3.20+ BTA fallback keeps IC and fails; per-subproject extra["kotlin.incremental"]="false" verified on Flutter 3.47.5)
- Fix: Flutter debugger jumps into `binding.dart` on hot reload with no error shown → slug: 2026/09/fix-flutter-debugger-jumps-into-binding-dart-on-hot-reload (root cause pinned: dwds 24.4.x sent PauseInterrupted for its internal breakpoint-reregistration pause on web hot reload; binding.dart:845 in 3.35.4 is BindingBase.postEvent; fixed in dwds 25.1.0+1, first stable Flutter 3.38.0; 3.35.x never patched)
